...
double ldf_Mult_smooth(int normalize, double *eta, int x, double*th, int lo, int hi) {
  ...
  // TODO: we shouldn't have to compute the sum
  if ((x<lo)||(x>hi)) { return NEGINF; }
  if (!normalize) return BOUNDPROB(log(th[x-lo]+eta[x]));
  //s = eta*(hi-lo+1);
  //for (i=hi-lo; i>=0; i--) { s+=th[i]; }
  s = th[hi-lo+1] + eta[0];
  return BOUNDPROB(log(th[x-lo]+eta[x]) - log(s));
}
...
int** load_discrete2(char*fname, int*dim1, int**dim2, int*max) {
  ...
  *dim1 = k;
  ...
  x     = (int **) malloc(*dim1 * sizeof(int*));
  *dim2 = (int *) malloc(*dim1 * sizeof(int));

  s   = (char *) malloc(1000000000 * sizeof(char));
  tmp = (int *) malloc(1000000000 * sizeof(int));
...
unsigned int*** load_discrete3(const char*fname, unsigned int*dim1, unsigned int**dim2, unsigned int***dim3, unsigned int*max) {
  FILE *f;
  unsigned int***x;
  //char*s;
  ...
  *dim2 = (unsigned int *) malloc(*dim1 * sizeof(unsigned int));
  k = 0;
  i = 0;
  f = fopen(fname, "r");
  while (!feof(f)) {
  ...
  *dim3 = (unsigned int **) malloc(*dim1 * sizeof(unsigned int*));
  for (k=0; k<*dim1; k++) {
    (*dim3)[k] = (unsigned int *) malloc((*dim2)[k] * sizeof(unsigned int));
  }
  k = 0;
  i = 0;
  j = 0;
  ...
  x = (unsigned int ***) malloc(*dim1 * sizeof(unsigned int**));
  //fprintf(stderr,"dim1 = %d\n", *dim1);
  for (k=0; k<*dim1; k++) {
    //fprintf(stderr,"dim2[%d] = %d\n", k, (*dim2)[k]);
    x[k] = (unsigned int **) malloc((*dim2)[k] * sizeof(unsigned int*));
    for (i=0; i<(*dim2)[k]; i++) {
      //fprintf(stderr,"dim3[%d][%d] = %d\n", k, i, (*dim3)[k][i]);
      x[k][i] = (unsigned int *) malloc((*dim3)[k][i] * sizeof(unsigned int));
    }
  }
...
int**** load_discrete4(char*fname, int*dim1, int**dim2, int***dim3, int****dim4, int*max) {
  ...
  *dim3 = (int **) malloc(*dim1 * sizeof(int*));
  for (k=0; k<*dim1; k++) {
    (*dim3)[k] = (int *) malloc((*dim2)[k] * sizeof(int));
  }
  ...
  *dim4 = (int ***) malloc(*dim1 * sizeof(int**));
  for (k=0; k<*dim1; k++) {
    (*dim4)[k] = (int **) malloc((*dim2)[k] * sizeof(int*));
    for (i=0; i<(*dim2)[k]; i++) {
      (*dim4)[k][i] = (int *) malloc((*dim3)[k][i] * sizeof(int));
    }
  }
  ...
  for (k=0; k<*dim1; k++) {
    //fprintf(stderr,"dim2[%d] = %d\n", k, (*dim2)[k]);
    x[k] = (int ***) malloc((*dim2)[k] * sizeof(int**));
    for (i=0; i<(*dim2)[k]; i++) {
      //fprintf(stderr,"dim3[%d][%d] = %d\n", k, i, (*dim3)[k][i]);
      x[k][i] = (int **) malloc((*dim3)[k][i] * sizeof(int*));
      for (j=0; j<(*dim3)[k][i]; j++) {
        //fprintf(stderr,"dim4[%d][%d][%d] = %d\n", k, i, j, (*dim4)[k][i][j]);
        x[k][i][j] = (int *) malloc((*dim4)[k][i][j] * sizeof(int));
      }
    }
  }
